...
layout(set = 0, binding = 2) uniform sampler2D ditherTexture;
...
void main() {
    ...
    float depth1 = texture(depthTexture, texCoord).x;
    if (depth1 >= 0.999999) {
        // skip background
        fragColor = vec4(1.0);
        return;
    }
    float depth2 = texture(depthTexture, texCoord + vec2(pixelShift.x, 0.0)).x;
    float depth3 = texture(depthTexture, texCoord - vec2(pixelShift.x, 0.0)).x;
    float dDepthdX = minabs(depth2 - depth1, depth1 - depth3);
    float depth4 = texture(depthTexture, texCoord + vec2(0.0, pixelShift.y)).x;
    float depth5 = texture(depthTexture, texCoord - vec2(0.0, pixelShift.y)).x;
    ...
    // Sampling pattern
    float patternPos1 = texture(ditherTexture, gl_FragCoord.xy * 0.25).x * 15.0 / 16.0;
    float patternPos2 = texture(ditherTexture, gl_FragCoord.xy * 0.25 + vec2(0.25, 0.0)).x * 15.0 / 16.0;

    patternPos1 += texture(ditherTexture, gl_FragCoord.xy * 0.25 * 0.25).x / 16.0;
    patternPos2 += texture(ditherTexture, gl_FragCoord.xy * 0.25 * 0.25 + vec2(0.25, 0.0)).x / 16.0;
    ...
    for (int i = 0; i < 16; ++i) {
        ...
        float sampledDepth = texture(depthTexture, sampleCoord).x;
